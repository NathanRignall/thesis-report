...
Then, using a small machine learning model, the system predicts the likely performance with high degrees of accuracy for different prefill configurations.
...
Instead, simple operations are used, which allows us to inspect performance and modify the calculations as necessary.
The Shallowsim simulator also already has native support for Mixture of Expert models (our key novel design goal).
...
\todo{
\textit{This is just describing how shallowsim does its maths, will finish last as it is not strictly necessary for the report}
}
...
As-built Shallowsim is designed to profile several different parallelism configurations and batch sizes, then report back the overall fastest.
...
To do this, we extract all the mathematical operations present in the existing code and form procedures with specific input parameters, e.g., $\texttt{prefill\_alltoall}(\texttt{gpu}, \texttt{seq\_len}, \texttt{tp\_num})$.
