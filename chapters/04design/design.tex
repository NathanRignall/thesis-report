...
This chapter outlines the steps taken to design and implement a new heterogeneous scheduler for LLM workloads, with support for Mixture-of-Experts models.
...
Next, we outline the choice of simulator, of which can be used for expected throughput workload modelling of the system.
Using this, we describe objectives for a bounded constraint optimisation problem, as a subset of the overall scheduler.
Then, we  describe several scheduling assumptions which can be used as optimisation strategies.
...
Given a list of islands, we must then assign either prefill or decode work (and configure appropriate parallelism levels with respect to MoE), maximising a defined objective.
In this case we are optimising for cost efficiency, so throughout / cost is a good metric.
