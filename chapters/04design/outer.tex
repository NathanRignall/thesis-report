...
Instead we use a refined Bayesian Optimisation search method with two different methods for searching, \textit{direct} and \textit{divided}.


%also simplier

%Given the BO approach we propose 
...
\todo{
\begin{itemize}
\item A little more on why BO and not RL.
\end{itemize}
}
...
Configurations correspond to a fixed number of islands, with each candidate solution representing a specific allocation.
The number of GPUs in each island are directly part of the output space, where specific variables represent the GPU count per island `slot'.
...
The \textit{divider} island generation method uses a more abstract representation that substantially reduces the dimensionality of the problem.
