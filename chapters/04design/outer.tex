...
This applies to our-use case where the \textit{inner loop} and constraint solver is effectively black-box.
...
The optimisation process begins with a warm-start phase, where a small number of valid but randomly generated layouts are evaluated.
...
Additionally, in some scenarios many parameters for the slots may optimally be zero (singular large island), creating a sparse problem that can be difficult for the optimiser to navigate.
Finally, keeping the total number of GPUs used across the slots below the total inventory count can also be difficult.
