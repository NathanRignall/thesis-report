...
Intro to Bayesian Optimisation
...
In the direct island generation approach, islands are explicitly defined within the Bayesian Optimisation process.
Configurations correspond to a fixed number of islands, with each candidate solution representing a specific allocation of resources.
The number of GPUs in each island is directly part of the output space, where specific variables represent the GPU count per slot.
This setup provides precise control over how resources are grouped, but it also requires well-defined constraints to ensure that all generated configurations are valid.

\subsubsection*{Parameters}

\begin{itemize}
    \item \( G \): the set of GPU types, e.g., \( G = \{\text{A100}, \text{V100}, \ldots\} \)
    \item \( K \): the number of GPU slots per type
    \item \( x_{g,k} \): the integer decision variable representing the number of GPUs of type \( g \in G \) assigned to slot \( k \in \{1, \ldots, K\} \)
    \item \( I_g \): the total inventory of GPU type \( g \in G \)
\end{itemize}

Each parameter \( x_{g,k} \) is bounded:

\[
x_{g,k} \in \{0, 1, \ldots, I_g\}, \quad \forall g \in G, \; \forall k \in \{1, \ldots, K\}
\]

\subsubsection*{Constraints}

For each GPU type \( g \in G \), we enforce that the total number of GPUs allocated across all \( K \) slots does not exceed the inventory \( I_g \):

\[
\sum_{k=1}^{K} x_{g,k} \leq I_g, \quad \forall g \in G
\]
...
\todo{
	...
	How the first version of this worked...
...
The divider island generation method uses a more abstract representation that substantially reduces the dimensionality of the problem.
...
This approach becomes computationally expensive due to the high number of variables and the need for constraints to ensure valid configurations.
...
An important advantage of this method is that it avoids the need for explicit resource constraints, which are otherwise necessary to enforce inventory limits and prevent infeasible configurations.
By shifting from low-level allocation to higher-level distribution strategies, divider island generation maintains control over performance-critical characteristics while offering a more scalable and interpretable optimisation process.
...
\begin{figure}[ht]
    ...
    \caption[Graph of displaying the effect of changing \(S_{\text{skew}}\)]
    {Graph of displaying the effect of changing \(S_{\text{skew}}\).}
...
\begin{itemize}
    \item \( G \): the set of GPU types, e.g., \( G = \{\text{A100}, \text{V100}, \ldots\} \)
    \item \( n_g \): the number of islands for GPU type \( g \in G \)
    \item \( s_g \): a scalar value representing the skew or balance in GPU allocation across the \( n_g \) islands for GPU type \( g \)
    \item \( I_g \): the total available inventory of GPU type \( g \)
\end{itemize}

The optimization parameters for each GPU type \( g \in G \) are:
\[
(n_g, s_g) \in \mathbb{Z}_{\geq 1} \times [0, 1]
\]
...
These two parameters per GPU type define the entire resource distribution strategy, allowing the Bayesian Optimization process to search over high-level allocation patterns while implicitly satisfying inventory constraints by design.
