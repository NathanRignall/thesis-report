...
\label{chap:throughput-modelling}
...
Hence, we need a method to accurately calculate the throughput of a system, with a variety of input sequence lengths.
...
The expected prefill throughput for workload $w$, denoted $\mathbb{E}[R_{\text{prefill}} \mid w]$ (expectation), is therefore given by:
...
The overall expected throughput is therefore given by:
...
%    {Sample expected throughput, calculated using the workload shown in \autoref{fig:04-sample-prefill-pdf} and modelled using Shallowsim.}
%    \label{fig:04-sample-expected-throughput}
%\end{figure}

\subsection{Throughput Allocation}

Previous calculations assumed that a singular GPU island is contributing to the overall throughput of the inferance system.
...
%    \centering
%    \includegraphics[width=\textwidth,height=5cm]{example-image-a}
%    \caption[Example throughput allocation mapping]
%    {Example throughput allocation mapping.}
