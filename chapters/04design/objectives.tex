...
%Formally, let $s$ denote the sequence length, $S$ the set of all possible sequence lengths, $p(s)$ the probability of observing a request of length $s$, and $R_{\text{prefill}}(s)$ the measured prefill throughput at that length (in requests per second).
%The expected prefill throughput, denoted by $\mathbb{E}[R_{\text{prefill}}]$, is then given by the following expression:
%\[
%\mathbb{E}[R_{\text{prefill}}] = \sum_{s \in S} R_{\text{prefill}}(s) \cdot p(s)
%\]
...
We normalise these allocations so that
\[
\sum_{s\in S} x_i(s) = 1
...
\]
Since end‐to‐end request processing cannot exceed its slowest stage, the overall expected throughput is simply the bottleneck of these two values:
\[
