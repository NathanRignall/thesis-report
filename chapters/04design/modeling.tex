...
For example, one would expect code prompts and responses to have different input sequence and decode lengths than to that of conversation prompts.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\textwidth]{traces_sample_pdfs.pdf}
...
%\begin{figure}[ht]
%    \centering
%    \includegraphics[width=0.8\textwidth]{traces_decode_sample.pdf}
%    \caption[Sample probability distribution of decode lengths]
%    {Sample probability distribution of decode length. Modelled using a pdf where $mu=100$, $sigma=200$ and truncated/normalised to the range \(0 \leq s \leq 500\). Average decode length (100) and max decode length (500) are extracted for use in the scheduler.}
%    \label{fig:04-sample-decode-pdf}
%\end{figure}
